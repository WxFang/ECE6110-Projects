...
int main(int argc, char* argv[]) {
	...
	for(u32 i = 0; i < d.RightCount(); ++i) {
		...
		sinks.Add(sinHelper.Install(d.GetRight(i)));
	}
	...
	for(u32 i = 0; i < d.LeftCount(); ++i) {
        ...
        AddressValue remoteAddress(InetSocketAddress(d.GetRightIpv4Address(i), 5000));
        bulkSender.SetAttribute("Remote", remoteAddress);
        sourceApps.Add(bulkSender.Install(d.GetLeft(i)));
	}
