...
void EnqueueAtRed(Ptr<const QueueItem> item) {
	...
	//if(tcp.GetDestinationPort() >= 50005){
	//	std::cout << Simulator::Now() << "\t" << tcp.GetDestinationPort () << "\t" << tcp.GetSequenceNumber() << "\t" << std::endl;
...
void DroppedAtRed(Ptr<const QueueItem> item) {
	...
	// if(tcp.GetDestinationPort () >= 50005 && tcp.GetDestinationPort () <= 50008){
	//  	std::cout << Simulator::Now() << "\t" << tcp.GetDestinationPort () << "\t" << tcp.GetSequenceNumber() << "\t" << std::endl;
	//  } 
}
...
int main(int argc, char* argv[]) {
	...
	Ptr<QueueDisc> redQueue0 = queueDiscs.Get(0);
	Ptr<QueueDisc> redQueue1 = queueDiscs.Get(1);

	//setup traces
	// redQueue0->TraceConnectWithoutContext("Enqueue", MakeCallback(&EnqueueAtRed));
	// redQueue0->TraceConnectWithoutContext("Dequeue", MakeCallback(&DequeueAtRed));
	// redQueue0->TraceConnectWithoutContext("Drop", MakeCallback(&DroppedAtRed));
	...
	if(writeForPlot) {
		...
		//Simulator::ScheduleNow(&CheckQueueSize, redQueue0);
